  ...
  const columns = useMemo<ColumnDef<Book>[]>(
    () => [
      ...
      },
      {
        ...
        header: 'ISBN-13',
        cell: ({ row }) => (
          <div className="p-3 text-amber-100/90 text-xs font-mono" style={fontStyle}>
            {row.getValue('isbn13')}
          </div>
        ),
      },
        ...
        <div className="relative">
          ...
          <div className="hidden md:block relative">
            <button
              ...
              className={`w-full px-4 py-2 bg-amber-200/10 border rounded-xs text-xs font-medium flex items-center justify-between gap-2 transition-all ${
                  ...
                  <div className="px-4 py-3 border-b border-amber-200/20 flex items-center justify-between">
                    ...
                    {selectedCategories.size > 0 && (
                      <button
                        ...
                        className="text-xs text-amber-200/70 hover:text-amber-200 transition-colors"
                        style={fontStyle}
                      >
                        Clear all
                      </button>
                    ...
                    {uniqueCategories.map((category) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full px-4 py-2.5 text-left text-xs flex items-start gap-3 hover:bg-amber-200/10 transition-colors ${
          ...
          <div className="md:hidden">
            <button
              ...
              className={`w-full px-4 py-2 bg-amber-200/10 border rounded-xs text-xs font-medium flex items-center justify-between gap-2 ${
                selectedCategories.size > 0
                  ? 'border-amber-200/70 bg-amber-200/20 text-amber-100'
          ...
          >
            <div className="px-4 py-4 border-b border-amber-200/20 flex items-center justify-between flex-shrink-0">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  onClick={() => setIsCategoryFilterOpen(false)}
                  className="p-1 text-amber-200/60 hover:text-amber-200 hover:bg-amber-200/10 rounded-xs transition-colors"
                  aria-label="Close"
                >
                  <X className="w-5 h-5" />
                </button>
              ...
              {uniqueCategories.map((category) => {
                ...
                return (
                  <button
                    key={category}
                    onClick={() => handleCategoryToggle(category)}
                    className={`w-full px-4 py-3 text-left text-sm flex items-start gap-3 hover:bg-amber-200/10 transition-colors border-b border-amber-200/10 ${
                      isSelected ? 'bg-amber-200/15' : ''
                    }`}
                    style={fontStyle}
                  >
            ...
            <div className="px-4 py-4 border-t border-amber-200/20 flex-shrink-0">
              <button
                onClick={() => setIsCategoryFilterOpen(false)}
                className="w-full px-4 py-2.5 bg-amber-200/20 hover:bg-amber-200/30 text-amber-100 rounded-xs border border-amber-200/50 transition-all hover:border-amber-200/70 hover:shadow-sm text-sm font-semibold"
                style={fontStyle}
              >
                Done
              </button>
